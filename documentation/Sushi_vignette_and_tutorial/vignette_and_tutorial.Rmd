...
# Set up inputs ----
filtered_counts <- data.table::fread(file.path(local_test_data_path, "filtered_counts.csv"))
CB_meta <- data.table::fread(file.path(local_test_data_path, "CB_meta.csv"))
pseudocount <- 0
...
If pool level growth annotations are provided, the computed L2FCs can be corrected to remove variance associated with different pool growth conditions.
...
# Set up inputs ----
normalized_counts <- data.table::fread(file.path(local_test_data_path, "normalized_counts.csv"))
...
To apply the growth correction, there must be a column in the data frame that describes the growth pattern of a cell line or pool.

```{r example growth correct, echo = T, eval = T}
# Load into environment ----
source(file.path(local_sushi_path, "scripts", "growth_correction", "growth_correction.R"))
...
qc_path <- data.table::fread(file.path(local_test_data_path, "qc_table.csv"))
...
# Set up inputs ----
l2fc <- data.table::fread(file.path(local_test_data_path, "l2fc.csv"))
sig_cols <- c("cell_set","pert_name","pert_dose","pert_dose_unit","day")
...
drc_table = data.table::fread(file.path(local_test_data_path, "drc_table.csv"))
...
# Set up inputs ----
depmap_file_path <- file.path(local_test_data_path, "prism_biomarker_public_0625.h5")
collapsed_l2fc <- data.table::fread(file.path(local_test_data_path, "collapsed_l2fc.csv"))
cell_line_cols <- c("depmap_id","lua","pool_id")
sig_cols <- c("cell_set","pert_name","pert_dose","pert_dose_unit","day")
