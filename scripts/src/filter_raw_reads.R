...
#' Fetch and Subset Skipped Well Data
#'
#' This function retrieves assay plate skipped well data from the Clue.io API for a given screen.
#' It then subsets the response to include only the following columns:
#' \itemize{
#'   \item \code{pcr_well}: The assay well position (renamed from \code{assay_well_position}).
#'   \item \code{pert_plate}: Pert plate information.
#'   \item \code{replicate}: Replicate information.
#'   \item \code{pool_id}: Pool identifier.
#' }
#' The API key is passed in the headers along with additional required keys.
#'
#' @param screen A character string specifying the screen identifier.
#' @param api_key A character string containing the API key.
#'
#' @return A dataframe containing the subset of columns.
#'
#' @examples
#' \dontrun{
#'   api_key <- "YOUR_API_KEY_HERE"
#'   result <- fetch_plate_skipped_well("MTS027", api_key)
#'   head(result)
#' }
#'
#' @export
fetch_skipped_wells <- function(screen, api_key) {
  base_url <- "https://api.clue.io/api/v_e_assay_plate_skipped_well"

  # Create the filter list and convert it to JSON
  filter_list <- list(where = list(screen = screen))
  filter_json <- jsonlite::toJSON(filter_list, auto_unbox = TRUE)

  # URL encode the JSON filter and construct the full URL
  full_url <- paste0(base_url, "?filter=", URLencode(filter_json))

  # Set up the required headers
  my_headers <- httr::add_headers(
  ...
  response <- httr::GET(full_url, my_headers)
  ...
  # Parse the JSON content and convert it into a normal dataframe
  content_text <- httr::content(response, as = "text", encoding = "UTF-8")
  data_list <- jsonlite::fromJSON(content_text, flatten = TRUE)
  data_df <- as.data.frame(data_list)

  # Define the required columns and subset the data
  required_columns <- c("assay_well_position", "pert_plate", "replicate", "pool_id", "replicate")
  missing_cols <- setdiff(required_columns, names(data_df))
  if (length(missing_cols) > 0) {
    warning("The following expected columns are missing from the response: ",
            paste(missing_cols, collapse = ", "))
  }

  common_cols <- intersect(required_columns, names(data_df))
  data_subset <- data_df[, common_cols, drop = FALSE]

  # Drop rows where the pool_id and/or replicate are missing
  data_subset <- data_subset[complete.cases(data_subset[, c("pool_id", "replicate")]), ]

  # Rename assay_well_position to pcr_well if present
  if ("assay_well_position" %in% names(data_subset)) {
    names(data_subset)[names(data_subset) == "assay_well_position"] <- "pcr_well"
  }
