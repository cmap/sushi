...
def build_parser():
    ...
    parser.add_argument(
        "--build_path",
        "-b",
        help="Build path with SUSHI level 3, 4, and 5 data.",
        required=True,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--days",
        "-d",
        help="Day timepoints to drop from output data separated by commas.",
    )
    parser.add_argument(
        "--config",
        "-c",
        help="Config file for project.",
        required=True,
        default="config.json",
    )
    return parser
...
def write_key(df):
    ...
    grouped_df = (
        distinct_df.groupby(
            [col for col in distinct_df.columns if col not in exclude_columns]
        )
        .nunique()
        .reset_index()
    )
...
def main(args):
    ...
    for ind, dataset in enumerate(datasets):
        ...
        missing_columns = [
            col for col in ["x_project_id", "pert_plate"] if col not in dataset.columns
        ]
        ...
        if missing_columns:
            ...
            raise ValueError(
                f"Columns '{missing_cols_str}' not found in the '{dataset_name}' dataset. Cannot proceed."
            )
        else:
    ...
    if args.days:
        ...
        for dataset in datasets:
            dataset.drop(
                dataset[dataset["pert_time"].isin(pert_time_to_drop)].index,
                inplace=True,
            )

    # Setting columns
    print("Reformatting columns...")
    sample_meta = sample_meta.assign(
    ...
    level_3 = level_3.assign(
        pert_vehicle=pert_vehicle,
        pert_time_unit=pert_time_unit,
        pert_id=level_3["pert_iname"].str.upper(),
        prc_id=level_3["pert_iname"].str.upper(),
    )

    level_4 = level_4.assign(
        pert_vehicle=pert_vehicle,
        pert_time_unit=pert_time_unit,
        pert_id=level_4["pert_iname"].str.upper(),
        prc_id=level_4["pert_iname"].str.upper(),
    )

    level_5 = level_5.assign(
        pert_vehicle=pert_vehicle,
        pert_time_unit=pert_time_unit,
    ...
    sample_meta["pert_itime"] = (
        sample_meta["pert_time"].astype(str) + " " + sample_meta["pert_time_unit"]
    )
    sample_meta["pert_idose"] = (
        sample_meta["pert_dose"].astype(str) + " " + sample_meta["pert_dose_unit"]
    )

    level_3["pert_itime"] = (
        level_3["pert_time"].astype(str) + " " + level_3["pert_time_unit"]
    )
    level_3["pert_idose"] = (
        level_3["pert_dose"].astype(str) + " " + level_3["pert_dose_unit"]
    )

    level_4["pert_itime"] = (
        level_4["pert_time"].astype(str) + " " + level_4["pert_time_unit"]
    )
    level_4["pert_idose"] = (
        level_4["pert_dose"].astype(str) + " " + level_4["pert_dose_unit"]
    )

    level_5["pert_time"] = (
        level_5["pert_time"].astype(str) + " " + level_5["pert_time_unit"]
    )
    level_5["pert_idose"] = (
        level_5["pert_dose"].astype(str) + " " + level_5["pert_dose_unit"]
    )

    # Define a mapping for renaming values
    trt_type_mapping = {"poscon": "trt_poscon", "negcon": "ctl_vehicle"}
