...
def run_backfill(build_dirs_file, workspace_path):
    ...
    for i, build_dir in enumerate(build_dirs, 1):
        ...
        for script_name in SCRIPTS_TO_RUN:
            ...
            # The command is just the launch script and the module to run.
            # We do NOT pass the build_dir as an argument here.
            ...
            try:
                ...
                # 2. Add the BUILD_DIR variable that launch_jobs.sh expects.
                proc_env['BUILD_DIR'] = os.path.abspath(build_dir)

                # 3. Pass this modified environment to the subprocess.
...
def main():
    ...
    parser.add_argument('build_dirs_file', type=str, help='Path to the text file with build directories.')
    parser.add_argument('--workspace', type=str, required=True, help="Path to your pipeline code checkout.")
