...
cell_line_cols= c('depmap_id', 'forward_read_barcode', 'barcode_id')
cell_line_meta <- cell_lines_df %>%
  rename("forward_read_barcode" = "dna_sequence", "lua" = "barcode_id") %>% dplyr::select(any_of(c(cell_line_cols)))
