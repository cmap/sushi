...
if (!str_detect(cb_ladder, ".csv")){ 
  control_bc_df <- get_cell_api_info(paste(api_url,"v_control_barcodes", sep = "/"), api_key, 
                                     filter = list(where = list(set = cb_ladder), fields = c("sequence", "name", "log_dose", "set")))
} else {
...
if (nrow(control_bc_df) > 0 & !str_detect(cb_ladder, ".csv") ) {
  CB_meta <- control_bc_df %>% rename("forward_read_barcode" = "sequence", "cb_name" = "name", "cb_ladder" = "set",
                                      "log_10_dose" = "log_dose")
