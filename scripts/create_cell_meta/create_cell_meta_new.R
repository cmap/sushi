...
source("create_cell_meta/create_cell_meta_functions_new.R")
...
if (args$verbose) {
  message(sprintf("Retrieved %d cell sets", nrow(cell_sets_df)))
  message(sprintf("Retrieved %d cell pools", nrow(cell_pools_df)))
  message(sprintf("Retrieved %d cell lines", nrow(cell_lines_df)))
}
...
if (args$verbose) {
  message(sprintf("Prepared metadata for %d cell lines", nrow(cell_line_meta)))
}
...
if (args$verbose) {
  message(sprintf("Created long-form cell set metadata with %d rows", nrow(cell_set_meta_long)))
}
