...
normalized_counts= read_data_table(args$normalized_counts, header= TRUE, sep= ',', data.table= FALSE)
...
if (args$filter_failed_lines) {
  ...
  qc_data = read_data_table(args$qc_path, header= TRUE, sep= ',', data.table= FALSE)
