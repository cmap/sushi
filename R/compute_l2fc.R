...
compute_l2fc = function(normalized_counts, 
  ...
  init_tr_excluded_columns= c('pcr_plate','pcr_well', 'Name', 'log2_dose', 'cb_intercept',
                              'profile_id', 'tech_rep', 'n', 'log2_n', 'normalized_n', 'log2_normalized_n',
                              'flag', count_col_name)
  tr_excluded_columns= init_tr_excluded_columns[!init_tr_excluded_columns %in% sig_cols]
  
  # collapse tech reps
  collapsed_tech_rep= normalized_counts %>%
    dplyr::filter(!(trt_type %in% c("empty", "", "CB_only")) & !is.na(trt_type), !is.na(CCLE_name)) %>%
    dplyr::group_by_at(setdiff(names(.), tr_excluded_columns)) %>% 
